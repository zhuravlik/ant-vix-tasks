 ...
 * @author anton
 */
public class Unpause extends VixAction {
    @Override
    public void executeAction(int vmHandle) {
        log("Sending unpause command", Project.MSG_INFO);

        int jobHandle = Vix.VIX_INVALID_HANDLE;

        jobHandle = LibraryHelper.getInstance().VixVM_Unpause(vmHandle, 0, Vix.VIX_INVALID_HANDLE, null, null);
