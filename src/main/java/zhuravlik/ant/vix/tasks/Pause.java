 ...
 * @author anton
 */
public class Pause extends VixAction {

    @Override
    public void executeAction(int vmHandle) {
        log("Sending pause command", Project.MSG_INFO);

        int jobHandle = Vix.VIX_INVALID_HANDLE;

        jobHandle = LibraryHelper.getInstance().VixVM_Pause(vmHandle, 0, Vix.VIX_INVALID_HANDLE, null, null);
